...
# 定义基础参数
default_model_path = "/root/ld/ld_model_pretrain/Qwen2.5-3B-Instruct"
default_inference_batch_size = 8
default_tensor_parallel_size = 1

# 实际函数定义
def interface(choice, model_type, model_path, model_api, inference_batch_size, tensor_parallel_size,
              save_question_json, input_question_json, save_react_qa_json, 
              save_complex_question_json, complex_example_json, 
              input_complex_question_json, save_complex_react_qa_json, except_comple_react_json):
    # 根据模型类型选择正确的模型输入
    model = model_path if model_type == "本地模型" else model_api
    ...
    if choice == "简单Agent问题构造":
        return get_question(model, save_question_json, inference_batch_size,tensor_parallel_size)
    elif choice == "简单Agent训练数据构造":
        return get_react_data(model, input_question_json, save_react_qa_json, inference_batch_size,tensor_parallel_size)
    elif choice == "复杂Agent 问题构造":
        return get_complex_question(model, save_complex_question_json, complex_example_json, inference_batch_size,tensor_parallel_size)
    elif choice == "复杂Agent 训练数据构造":
        return get_complex_react_data(model, input_complex_question_json, save_complex_react_qa_json, inference_batch_size, except_comple_react_json,tensor_parallel_size)
...
with gr.Blocks() as demo:
    ...
    inputs=[
        ...
        model_api_input,  # model_api
        inference_batch_size_input,  # inference_batch_size
        tensor_parallel_size_input,  # tensor_parallel_size
        save_question_json_input,  # save_question_json
        input_question_json_input,  # input_question_json
        save_react_qa_json_input,  # save_react_qa_json
        save_complex_question_json_input,  # save_complex_question_json
        complex_example_json_input,  # complex_example_json
        input_complex_question_json_input,  # input_complex_question_json
        save_complex_react_qa_json_input,  # save_complex_react_qa_json
        except_comple_react_json_input  # except_comple_react_json
    ],
    outputs=output
)
