...
def get_question(model_path,save_question_json,inference_batch_size):
...
def get_complex_question(model_path,save_complex_question_json, complex_example_json, inference_batch_size):
    if 'llm' not in locals():
        llm = LLM(
            model=model_path,
            tensor_parallel_size=8,
...
def get_react_data(model_path,input_question_json, save_react_qa_json,inference_batch_size):
    if 'llm' not in locals():
        llm = LLM(
            model=model_path,
            tensor_parallel_size=8,
...
def get_complex_react_data_batch(model_path,input_complex_question_json, save_complex_react_qa_json,  except_comple_react_json,inference_batch_size=None):
    if 'llm' not in locals():
        llm = LLM(
            model=model_path,
            tensor_parallel_size=8,
